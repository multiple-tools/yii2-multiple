<?php
    ...
    namespace umono\multiple\tools\image;
    ...
    class PosterHandler
    {
         ...
         * Poster constructor.
         *
         * @param string | int $width                  海报宽度
         * @param string | int $height                 海报高度
        ...
        public function getPosition($margin = 0, $image_w = 0, $image_h = 0)
        {
            return [
                ...
                self::POSITION_RIGHT_BOTTOM     => [
                    $this->background_image_w - $margin - $image_w,
                    $this->background_image_h - $margin - $image_h,
                ],
                self::POSITION_TOP_CENTER       => [$this->background_image_center_x - ($image_w / 2), $margin],
                self::POSITION_CENTER           => [
                    $this->background_image_center_x - ($image_w / 2),
                    $this->background_image_center_y - ($image_h / 2),
                ],
        ...
        protected function createBg($width, $height, $background_image_color)
        {
            ...
            $background_image_color = array_values($background_image_color);

            $this->background_image = imagecreatetruecolor($width, $height);
            $canvas                 = imagecolorallocate(
                $this->background_image, $background_image_color[0], $background_image_color[1],
        ...
        public function addImage($img_path, $xy = [0, 0], $size_wh = [100, 100])
        {
            ...
            @imagecopyresized(
                $this->background_image, $img, $xy[0], $xy[1], 0, 0, $size_wh[0] - 1, $size_wh[1] - 1, $l_w, $l_h);
            imagedestroy($img);
            return $this;
        }
        ...
        {
            imagecopyresized(
                $this->background_image, $imageResource, $xy[0], $xy[1], 0, 0, $size_wh[0], $size_wh[1], $size_wh[0],
                $size_wh[1]);
        ...
        public function addQrCode($text, $size_wh = [100, 100], $position = 0, $xy = [0, 0])
        {
            ...
            if (in_array($position, array_keys($this->getPosition()))) {
                ...
                $xy                    = [
                    $this->qrcode_position[$position][self::X],
                    $this->qrcode_position[$position][self::Y],
                ];
            }
        ...
        public function addUser($head_portrait, $nickname, $position = 0)
        {
            ...
            $string = $this->getStrMaxLen(
                $user_name, $nickname['font_path'], $nickname['font_size'], $this->max_nickname_width);
            ...
            for ($i = 0; $i < ceil($text_count_len / $allow_max_text_len); $i++) {
                if ($i >= ($this->max_nickname_line_number - 1) && ($i * $allow_max_text_len + $allow_max_text_len) < $text_count_len) {
                    $str         = mb_substr(
                            $user_name, $i * $allow_max_text_len, $allow_max_text_len - 1, "UTF-8") . '...';
                    $text_list[] = $str;
                    break;
                }
            ...
            if (!empty($head_portrait['img_path'])) {
                [$avatar_w, $avatar_h] = getimagesize($head_portrait['img_path']);
                $avatar = $this->createImageFromFile($head_portrait['img_path']);
                if ($head_portrait['is_circular']) {
                    $avatar = self::circularImg($avatar);
                }
                imagecopyresized(
                    $canvas, $avatar, $avatar_x, $avatar_y, 0, 0, $head_portrait['width'], $head_portrait['height'],
                    $avatar_w, $avatar_h);
                imagedestroy($avatar);
            }
            ...
            for ($i = 0; $i < count($text_list); $i++) {
                ...
                imagettftext(
                    $canvas, $nickname['font_size'], 0, $nickname_x, $nickname_y, $nickname['color'],
                    $nickname['font_path'], $text_list[$i]);
            }
            //5、将内容合并到大图中

            $position_xy                                        = $this->getPosition(
                $this->user_margin, $canvas_widht, $canvas_height);
            ...
            imagecopyresized(
                $this->background_image, $canvas, $position_xy[$position][self::X], $position_xy[$position][self::Y], 0,
                0, $canvas_widht, $canvas_height, $canvas_widht, $canvas_height);
            imagedestroy($canvas);
        ...
        public static function circularImg($image, $is_path = false, $saveName = '', $border = [])
        {
            ...
            if ($is_border > 0) {
                ...
                //头像边框颜色
                $border_image_color = imagecolorallocatealpha(
                    $border_image, $border_color['r'], $border_color['g'], $border_color['b'], 0);
